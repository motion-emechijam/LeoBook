...
async def get_bet_slip_count(page: Page) -> int:
    ...
    count_sel = await get_selector_auto(page, "fb_global", "betslip_count_badge")
...
async def clear_bet_slip(page: Page):
    ...
    try:
        if await get_bet_slip_count(page) > 0:
            ...
            # Open bet slip
            open_sel = await get_selector_auto(page, "fb_global", "slip_trigger_button")
            slip_opened = False
            ...
            # Clear all bets
            clear_sel = await get_selector_auto(page, "fb_global", "betslip_clear_all")
            bets_cleared = False
            ...
            if clear_sel:
                try:
                    if await page.locator(clear_sel).count() > 0:
                        ...
                        confirm_sel = await get_selector_auto(page, "fb_global", "confirm_bet_button")
                        if confirm_sel:
                            try:
